...
export default {
  ...
  watch: {
    async q (q) {
      ...
      this.searching = true
      this.results = await this.$content(this.$i18n.locale, {deep: true}).sortBy('position', 'asc').limit(12).search(q).fetch()
      this.searching = false
    }
  },
  mounted () {
  ...
  methods: {
    ...
    go () {
      ...
      const link = result.path.replace("/en/", "")
      const to = { name: 'slug', params: { slug: link !== 'index' ? link : undefined } }
